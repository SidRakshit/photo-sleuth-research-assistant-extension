...
function extractPageHtml() {
    ...
    try {
        ...
        console.log("CONTENT SCRIPT: outerHTML retrieved, length:", data.pageHtml ? data.pageHtml.length : 'null');
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "extractDataRequest") {
        ...
        try {
            ...
            console.log("CONTENT SCRIPT: Attempting to send extractedDataResponse back to popup. Error:", extractedData.error);
            chrome.runtime.sendMessage({ action: "extractedDataResponse", data: extractedData });
            console.log("CONTENT SCRIPT: Sent extractedDataResponse.");
        } catch (error) {
             console.error("CONTENT SCRIPT: Error processing extractDataRequest or sending response:", error);
             try {
                 chrome.runtime.sendMessage({ action: "extractionError", error: error.message });
             } catch (sendError) {
                 console.error("CONTENT SCRIPT: Failed to send error message back to popup:", sendError);
             }
        }
        // Removed 'return true;'
    }
